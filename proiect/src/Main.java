...
//                    System.out.println(petshop.getDogs());
                    service.initReportCaini();
                    petshop.getDogs().forEach(dog -> service.writeDataToReportCaini(dog));
                    service.initReportPisici();
                    Cat[] catList = new Cat[0];
                    catList = petshop.getCatsArray();
                    for(int i = 0; i < catList.length; i++){
                        service.writeDataToReportPisici(catList[i]);
                    }
//                    System.out.println(petshop.getCats());
                    System.out.println("Rapoartele au fost scrise!");
...
//                    petshop.showEmployeesOrderByName();
                    service.initReportAngajati();
                    List<Employee> employeeList = new ArrayList<>();
                    employeeList = petshop.getEmployeesByName();
                    employeeList.forEach(employee -> service.writeDataToReportAngajati(employee));
                    System.out.println("Rapoartele au fost scrise!");
                    System.out.println("Dati optiune:");
                    break;
                }
...
//                    System.out.println("Cainii predispusi la adoptie: ");
                    service.initReportCainiAdoptie();
                    for(Dog d : petshop.getDogs()){
                        if(d.adoptionGrade() > 7){
//                            System.out.println(d);
                            service.writeDataToReportCainiAdoptie(d);
                        }
                    }
                    System.out.println("Rapoartele au fost scrise!");
                    System.out.println("Dati optiune:");
